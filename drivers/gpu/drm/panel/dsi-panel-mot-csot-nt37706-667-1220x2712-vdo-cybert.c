...
#include "include/dsi-panel-mot-csot-nt37706-667-1220x2712-vdo-cybert-lhbm-alpha-v3.h"
				 ...
				 unsigned int level)
{
	...
	if (level == 4294967294 || level == 4294967293) {
		...
		if (hbm_state == 2) {
			/* LHBM ON: Send panel_lhbm_on command (44 bytes) */
			char lhbm_on_cmd[] = {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60,
					     0x02, 0x00, 0xC0, 0x30, 0x33, 0x09, 0x99, 0x00,
					     0x88, 0x02, 0x00, 0xC7, 0x01, 0x01, 0x09, 0x01,
					     0x00, 0x5F, 0x00, 0x01, 0x00, 0x40, 0x01, 0x00,
					     0x8B, 0x00, 0x00, 0x10, 0x01, 0x00, 0x87, 0x00,
					     0x02, 0x25, 0xbe, 0x80};

			/* Calculate alpha and update bytes 42-43 */
			unsigned int alpha_idx = (bl_level_local > 16000) ? 15999 : bl_level_local;
			unsigned int alpha = lhbm_alpha[alpha_idx];
			lhbm_on_cmd[42] = (alpha >> 8) & 0xFF;
			lhbm_on_cmd[43] = alpha & 0xFF;

			cb(dsi, handle, lhbm_on_cmd, sizeof(lhbm_on_cmd));
		} else {
			/* LHBM OFF: Send panel_lhbm_off commands */
			char lhbm_off_cmd1[] = {0xA9, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00,
						0x01, 0x00, 0x51, 0x00, 0x01, 0x3E, 0x80, 0x01,
						0x00, 0x51, 0x04, 0x05, 0x05, 0x54};
			char lhbm_off_cmd2[] = {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60,
						0x02, 0x00, 0xC0, 0x30, 0x33, 0x04, 0x57, 0x00,
						0x88, 0x02, 0x00, 0xC7, 0x01, 0x01, 0x01, 0x01,
						0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00,
						0x8B, 0x00, 0x00, 0x00, 0x01, 0x00, 0x87, 0x00,
						0x00, 0x20};

			/* Restore backlight in lhbm_off_cmd1 (using fill_backlight_cmd logic) */
			fill_backlight_cmd(bl_level_local, lhbm_off_cmd1);
...
static struct mtk_panel_para_table panel_lhbm_on[] = {
	{44, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x09, 0x99, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01, 0x01,
	      0x09, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x40, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x10, 0x01, 0x00, 0x87, 0x00, 0x02, 0x25, 0xbe, 0x80}},
};


static struct mtk_panel_para_table panel_lhbm_on_v3[] = {
	{50, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x09, 0x99, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01, 0x01,
	      0x09, 0x02, 0x08, 0xC1, 0x00, 0x01, 0x8F, 0xBF, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x10, 0x01, 0x00, 0x87, 0x00, 0x02, 0x25, 0xbe, 0x80,
	      0x02, 0x04, 0xcc, 0x01, 0x1, 0x00}},
};
...
static struct mtk_panel_para_table panel_lhbm_off[] = {
	...
	{42, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x04, 0x57, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01,
	      0x01, 0x01, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x00, 0x01, 0x00, 0x87, 0x00, 0x00, 0x20}},
};

static struct mtk_panel_para_table panel_lhbm_off_v3[] = {
	{22, {0xA9, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x51, 0x00, 0x01, 0x3E, 0x80, 0x01, 0x00, 0x51, 0x04, 0x05, 0x05, 0x54}},// apl:bit7  normal_bl: bit 13~14 aod_bl: 20~21
	{48, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x04, 0x57, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01,
	      0x01, 0x01, 0x02, 0x08, 0xC1, 0x00, 0x01, 0x8E, 0xFF, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x00, 0x01, 0x00, 0x87, 0x00, 0x00, 0x20,
	      0x02, 0x04, 0xcc, 0x01, 0x1, 0x00}},
};

static struct mtk_panel_para_table panel_lhbm_off_dc_v3[] = {
	{22, {0xA9, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x51, 0x00, 0x01, 0x3E, 0x80, 0x01, 0x00, 0x51, 0x04, 0x05, 0x05, 0x54}},// apl:bit7  normal_bl: bit 13~14 aod_bl: 20~21
	{48, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x04, 0x57, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01,
	      0x01, 0x01, 0x02, 0x08, 0xC1, 0x00, 0x01, 0x8E, 0xFF, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x00, 0x01, 0x00, 0x87, 0x00, 0x00, 0x20,
	      0x02, 0x04, 0xcc, 0x01, 0x1, 0x08}},
};
...
static int panel_lhbm_set_cmdq(struct lcm *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t on, uint32_t bl_level, uint32_t fps)
{
	...
	if (on) {
		...
		if(ctx->version  < 3) {
			...
			para_count = sizeof(panel_lhbm_on_v3) / sizeof(struct mtk_panel_para_table);
			pTable = panel_lhbm_on_v3;
		}
		...
		if(ctx->version  < 3) {
			...
			if (atomic_read(&ctx->dc_mode)) {
				para_count = sizeof(panel_lhbm_off_dc_v3) / sizeof(struct mtk_panel_para_table);
				pTable = panel_lhbm_off_dc_v3;
			} else {
				para_count = sizeof(panel_lhbm_off_v3) / sizeof(struct mtk_panel_para_table);
...
static struct mtk_panel_para_table panel_dc_off_v3[] = {
	...
	{6, {0xf0, 0x55, 0xaa, 0x52, 0x08, 0x00}},
	{27, {0xB3, 0x00, 0x04, 0x04, 0x5C, 0x04, 0x5C, 0x06, 0xCC, 0x06, 0xCC, 0x09, 0xD0, 0x09, 0xD0, 0x0D, 0xC8, 0x0D, 0xC8, 0x10, 0xAF, 0x10, 0xAF, 0x10, 0xB0, 0x10, 0xB0}},
	{6, {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x04}},
	{2, {0x6f, 0x01}},
	{2, {0xcc, 0x00}},
};

static struct mtk_panel_para_table panel_dc_on_v3[] = {
	{2, {0x6f, 0x01}},
	{2, {0x8b, 0x81}},
	{6, {0xf0, 0x55, 0xaa, 0x52, 0x08, 0x00}},
	{27, {0xB3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0xB0}},
...
static int pane_dc_set_cmdq(struct lcm *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t dc_state)
{
	...
	if (dc_state) {
		if(ctx->version < 3) {
			...
			para_count = sizeof(panel_dc_on_v3) / sizeof(struct mtk_panel_para_table);
			pTable = panel_dc_on_v3;
		}
	} else {
		if(ctx->version < 3) {
	...
	pr_info("%s: current_fps %d, ver[%d]\n", __func__, atomic_read(&ctx->current_fps), ctx->version);
	return 0;
}
...
static const struct of_device_id lcm_of_match[] = {
	{
	    .compatible = "csot,nt37706,vdo,667",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "csot_nt37706_667_vdo_1220_2712",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("CSOT NT37706 VDO 120HZ OLED Panel Driver");
