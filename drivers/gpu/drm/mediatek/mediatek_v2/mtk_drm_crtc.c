...
static struct mtk_drm_property mtk_crtc_property[CRTC_PROP_MAX] = {
	...
	{DRM_MODE_PROP_ATOMIC, "LCM_CE_ENABLE", 0, ULONG_MAX, 0},
};
...
int mtk_drm_crtc_set_panel_ce(struct drm_crtc *crtc, bool en)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	struct cmdq_pkt *cmdq_handle;
	...
	DDPINFO("%s:set LCM ce en:%d crtc %d\n", __func__, en, drm_crtc_index(crtc));
	...
	comp->funcs->io_cmd(comp, cmdq_handle, DSI_PANEL_CE_SET, &en);
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc)
{
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		if (mtk_crtc->hold_top_clk_by_early_power_on) {
			mtk_crtc->hold_top_clk_by_early_power_on = false;
			DDPINFO("%s crtc%d early power on skip top clk enable\n", __func__, crtc_id);
		} else {
			DDPINFO("%s crtc%d do top clk enable\n", __func__, crtc_id);
			mtk_drm_top_clk_prepare_enable(crtc->dev);
		}
